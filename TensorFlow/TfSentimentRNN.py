...
class SentimentRNN(object):
    ...
    def build(self):
        tf_x = tf.placeholder(tf.int32, shape=(
            self.batch_size, self.seq_len), name='tf_x')
        tf_y = tf.placeholder(tf.float32, shape=(self.batch_size), name='tf_y')
        tf_keepprob = tf.placeholder(
            tf.float32, shape=None, name='tf_keepprob')

        embedding = tf.Variable(tf.random_uniform(
            shape=(self.n_words, self.embed_size), minval=-1, maxval=1), name='embedding')
    ...
    def predict(self, X_data, return_proba=False):
        ...
        with tf.Session(graph=self.graph) as sess:
            ...
            for batch_x in self.create_batch_generator(X_data, None, batch_size=self.batch_size):
